...
route.post('/', async (req, res) => {
   ...
   try {
      ...
      // if email already exists return message user already exist please register another email
      if (!user) throw Error(`User [${email}] doesn't exists.`);
      ...
      if (!token) throw Error(`Something went wrong while generating token`);
      ...
      res.status(200).json({
       ...
       user: { _id: user.id, name: user.name, email: user.email },
         ...
         msg: `User [${user.email}] has logged in successfully.`,
      });
   } catch (e) {
...
route.get('/user', auth, async (req, res) => {
   try {
      // get id from user object stored in request coming from auth middleware.
